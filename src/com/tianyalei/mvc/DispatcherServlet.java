...
import com.tianyalei.mvc.bean.HandlerModel;
import com.tianyalei.mvc.util.Play;
...
/**
 * Created by wuwf on 17/6/28. 入口Sevlet
 */
public class DispatcherServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3383176744443901126L;
	...
	private void doHandlerMapping() {
		...
		for (Map.Entry<String, Object> entry : instanceMapping.entrySet()) {
			...
			for (Method method : methods) {
				...
				/*
				 * 以Search方法的这几个参数为例 @RequestParam("name") String name,
				 * HttpServletRequest request, HttpServletResponse response
				 * 未来在invoke时，需要传入类似这样的一个数组["abc", request,
				 * response]。"abc"即是在Post方法中通过request.getParameter("name")来获取
				...
				// "response"]
				String[] paramNames = Play.getMethodParameterNamesByAsm4(clazz,
						method);
				...
				for (int i = 0; i < annotations.length; i++) {
					...
					if (anns.length == 0) {
						...
						if (type == HttpServletRequest.class
							...
							// 参数没写@RequestParam注解，只写了String
							// name，那么通过java是无法获取到name这个属性名的
				...
				HandlerModel model = new HandlerModel(method, entry.getValue(),
						paramMap);
		...
		for (String className : classNames) {
			try {
				...
				if (clazz.isAnnotationPresent(Controller.class)) {
					...
					instanceMapping.put(lowerFirstChar(clazz.getSimpleName()),
							clazz.newInstance());
					...
					if (!"".equals(value.trim())) {
						...
						for (Class c : inters) {
							...
							instanceMapping.put(
									lowerFirstChar(c.getSimpleName()),
									clazz.newInstance());
							break;
						}
					}
